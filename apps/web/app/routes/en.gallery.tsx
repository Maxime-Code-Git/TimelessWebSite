...
export function meta(_args: Route.MetaArgs) {
  return [
    { title: "My gallery — Timeless" },
    { name: "description", content: "Welcome to your private gallery." },
...
export default function GalleryEn() {
  ...
  const countLabel = `${visible.length} of ${ALL_PHOTOS.length} photos`;
  const toggleLabel = expanded ? 'Show less' : 'See more';

  return (
    <div className={styles.container}>
      <Header lang="en" alternateLangHref="/fr/gallery" hideNav />

      <main>
        {/* ── Hero ────────────────────────────────────────── */}
        <section className={styles.hero}>
          <div className={styles.heroBg}>
            <div className={styles.photoPlaceholder}>Wedding cover photo</div>
          </div>
          <div className={styles.heroOverlay}></div>
          <div className={styles.heroContent}>
            <p className={styles.heroSubtitle}>Your day</p>
            <h1 className={styles.heroTitle}>Julie & Marc — June 14, 2025</h1>
            <p className={styles.heroText}>Welcome to your private gallery. Thank you for trusting us with such a beautiful day.</p>
          </div>
        </section>
          ...
          <div className={styles.shortcutsWrapper}>
            <div className={styles.shortcutsGrid}>
              <a href="#galerie-photo" className={styles.shortcutCard}>
                ...
                <span className={styles.shortcutTitle}>View photos</span>
              </a>
              <a href="#galerie-video" className={styles.shortcutCard}>
                <span className={`${styles.shortcutIcon} ${styles.shortcutIconVideo}`}>▶</span>
                <span className={styles.shortcutTitle}>View film</span>
              </a>
            </div>

            <div className={styles.downloadsArea}>
              <div className={styles.downloadsList}>
                <button className={styles.btnPrimary} onClick={(e) => e.preventDefault()}>Download all</button>
                <button className={styles.btnSecondary} onClick={(e) => e.preventDefault()}>Download photos</button>
                <button className={styles.btnSecondary} onClick={(e) => e.preventDefault()}>Download videos</button>
              </div>
              <p className={styles.downloadsNote}>.zip archive — ensure a stable connection.</p>
            </div>
          </div>
        </section>
            ...
            <div className={styles.photoGrid}>
              {visible.map((photo, i) => (
                <div key={photo.id} className={`${styles.photoItem} ${photo.ar === '4/3' ? styles.photoItemLandscape : styles.photoItemPortrait}`}>
                  ...
                  <button className={styles.downloadIcon} aria-label="Download this photo" onClick={(e) => e.preventDefault()}>↓</button>
                </div>
              ))}
            </div>
        ...
        <section id="galerie-video" className={styles.videoSection}>
          <div className={styles.videoWrapper}>
            <h2 className={styles.videoSectionTitle}>Your film</h2>
            <div className={styles.videoContainer}>
              <button className={styles.videoDownloadBtn} aria-label="Download film" onClick={(e) => e.preventDefault()} style={{
              ...
              {playing ? (
                ...
                <>
                  <div className={styles.videoPlaceholder}>Wedding film (cinema format)</div>
                  <button className={styles.playBtn} onClick={() => setPlaying(true)} aria-label="Play film">►</button>
                </>
              )}
            </div>
          </div>
        ...
        <section className={styles.footerLinkArea}>
          <p className={styles.footerLinkText}>Your gallery remains available for 24 months.</p>
          <a href="/en/contact" className={styles.footerContactLink}>An issue? Contact us</a>
        </section>
      </main>

      <Footer lang="en" hideNav />
      <ScrollTop />
    </div>
  );
}
